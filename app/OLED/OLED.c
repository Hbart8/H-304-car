 ...
 */
static uint8_t OLED_IsFont8x16(uint8_t FontSize) {
  return (FontSize == OLED_8X16 || FontSize == 16);
}

static uint8_t OLED_IsFont6x8(uint8_t FontSize) {
  return (FontSize == OLED_6X8 || FontSize == 8);
}

static uint8_t OLED_GetCharWidth(uint8_t FontSize) {
  if (OLED_IsFont8x16(FontSize)) {
    return 8;
  }
  if (OLED_IsFont6x8(FontSize)) {
    return 6;
  }
  return FontSize;
}

static uint8_t OLED_GetCharHeight(uint8_t FontSize) {
  if (OLED_IsFont8x16(FontSize)) {
    return 16;
  }
  if (OLED_IsFont6x8(FontSize)) {
...
static int16_t OLED_NormalizeY(int16_t Y, uint8_t FontSize) {
  if (OLED_IsFont8x16(FontSize) && Y >= 0 && Y <= 7) {
    return Y * 8;
  }
  return Y;
}

static const uint8_t *OLED_GetAsciiGlyph(char Char, uint8_t FontSize) {
  if (OLED_IsFont8x16(FontSize)) {
    return OLED_F8x16[(uint8_t)(Char - ' ')];
  }
  if (OLED_IsFont6x8(FontSize)) {
    return OLED_F6x8[(uint8_t)(Char - ' ')];
  }
  return NULL;
}
...
void OLED_ShowChar(int16_t X, int16_t Y, char Char, uint8_t FontSize) {
  Y = OLED_NormalizeY(Y, FontSize);
  if (OLED_IsFont8x16(FontSize)) // 字体为宽8像素，高16像素
  {
    /*将ASCII字模库OLED_F8x16的指定数据以8*16的图像格式显示*/
    OLED_ShowImage(X, Y, 8, 16, OLED_F8x16[Char - ' ']);
  } else if (OLED_IsFont6x8(FontSize)) // 字体为宽6像素，高8像素
  {
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize) {
  uint8_t CharWidth = OLED_GetCharWidth(FontSize);
  uint8_t i;
  for (i = 0; String[i] != '\0'; i++) // 遍历字符串的每个字符
  {
    /*调用OLED_ShowChar函数，依次显示每个字符*/
    OLED_ShowChar(X + i * CharWidth, Y, String[i], FontSize);
  }
}

void OLED_ShowString_Reverse(int16_t X, int16_t Y, char *String,
                             uint8_t FontSize) {
  uint8_t GlyphWidth = OLED_GetCharWidth(FontSize);
  uint8_t GlyphHeight = OLED_GetCharHeight(FontSize);
  uint8_t GlyphPages;
  int16_t CurrentX = X;
  uint8_t ReverseGlyph[16];
  uint8_t i;
  ...
  Y = OLED_NormalizeY(Y, FontSize);
  GlyphPages = GlyphHeight / 8;

  while (*String != '\0') {
    const uint8_t *Glyph = OLED_GetAsciiGlyph(*String, FontSize);
    if (Glyph == NULL) {
      break;
    }

    for (i = 0; i < GlyphWidth * GlyphPages; i++) {
      ReverseGlyph[i] = (uint8_t)(~Glyph[i]);
    }

    OLED_ShowImage(CurrentX, Y, GlyphWidth, GlyphHeight, ReverseGlyph);
    CurrentX += GlyphWidth;
    String++;
  }
}
...
void OLED_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length,
  ...
  for (i = 0; i < Length; i++) // 遍历数字的每一位
  {
    ...
    /*+ '0' 可将数字转换为字符格式*/
    OLED_ShowChar(X + (i + 1) * CharWidth, Y,
...
void OLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength,
  ...
  /*显示小数部分*/
  OLED_ShowNum(X + (IntLength + 2) * CharWidth, Y, FraNum, FraLength,
               FontSize);
}

void OLED_ShowFloat(int16_t X, int16_t Y, double Number, uint8_t IntLength,
                    uint8_t FraLength, uint8_t FontSize) {
  uint8_t CharWidth = OLED_GetCharWidth(FontSize);
  ...
  OLED_ShowChar(X + IntLength * CharWidth, Y, '.', FontSize);
  OLED_ShowNum(X + (IntLength + 1) * CharWidth, Y, FraNum, FraLength,
               FontSize);
}
...
/**
 * 函    数：OLED_Printf 内部辅助 —— 在光标处显示单个字符并推进X坐标
 */
static void OLED_Printf_PutChar(int16_t *pX, int16_t Y, char c,
                                uint8_t FontSize) {
  OLED_ShowChar(*pX, Y, c, FontSize);
  *pX += OLED_GetCharWidth(FontSize);
}

/**
 * 函    数：OLED_Printf 内部辅助 —— 显示有符号十进制整数
 */
static void OLED_Printf_PrintInt(int16_t *pX, int16_t Y, int num,
                                 uint8_t FontSize) {
  char buf[12];
  int i = 0;
  uint32_t n;
  ...
  if (num < 0) {
    OLED_Printf_PutChar(pX, Y, '-', FontSize);
    n = (uint32_t)(-(num + 1)) + 1;
  } else {
    n = (uint32_t)num;
  }
  ...
  if (n == 0) {
    OLED_Printf_PutChar(pX, Y, '0', FontSize);
    return;
  }
  ...
  while (i > 0) {
    OLED_Printf_PutChar(pX, Y, buf[--i], FontSize);
  }
}

/**
 * 函    数：OLED_Printf 内部辅助 —— 显示无符号十六进制整数
 */
static void OLED_Printf_PrintHex(int16_t *pX, int16_t Y, uint32_t num,
                                 uint8_t FontSize) {
  char buf[9];
  int i = 0;
  uint8_t digit;
...
/**
 * 函    数：OLED使用轻量级 printf 打印格式化字符串（不依赖标准C库）
 * 参    数：X
 ...
 * 参    数：... 格式化字符串参数列表
 * 支    持：%d(有符号整数), %u(无符号整数), %s(字符串), %c(字符),
 * %x/%X(十六进制), %%(百分号) 返 回 值：无 说
...
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize,
  ...
  for (p = format; *p != '\0'; p++) {
    if (*p == '%') {
      ...
      switch (*p) {
      case 'd':
        OLED_Printf_PrintInt(&X, Y, va_arg(arg, int), FontSize);
        break;
      case 'u':
        OLED_Printf_PrintInt(&X, Y, (int)va_arg(arg, unsigned int), FontSize);
        break;
      case 's': {
        char *s = va_arg(arg, char *);
        while (*s) {
          OLED_Printf_PutChar(&X, Y, *s++, FontSize);
        }
        break;
      }
      case 'c':
        OLED_Printf_PutChar(&X, Y, (char)va_arg(arg, int), FontSize);
        break;
      case 'x':
      case 'X':
        OLED_Printf_PrintHex(&X, Y, va_arg(arg, uint32_t), FontSize);
        break;
      case '%':
        OLED_Printf_PutChar(&X, Y, '%', FontSize);
        break;
      default:
        OLED_Printf_PutChar(&X, Y, '%', FontSize);
        OLED_Printf_PutChar(&X, Y, *p, FontSize);
        break;
      }
    } else {
      OLED_Printf_PutChar(&X, Y, *p, FontSize);
...
void OLED_Set_Pos(uint8_t x, uint8_t y) { OLED_SetCursor(y, x); }

void OLED_ClearLine(uint8_t line) {
  OLED_ClearArea(0, (int16_t) line * 8, 128, 8);
  OLED_UpdateArea(0, (int16_t) line * 8, 128, 8);
}

void OLED_DrawBMP(int16_t X, int16_t Y, uint8_t Width, uint8_t Height,
                  const uint8_t *BMP) {
  OLED_ShowImage(X, OLED_NormalizeY(Y, OLED_8X16), Width, Height, BMP);
}
